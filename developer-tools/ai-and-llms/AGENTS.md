...
This document provides essential context for AI and LLM agents when assisting developers with Amazon Location Service projects. Use this information to provide more accurate and helpful guidance.
...
- Static Maps: Pre-rendered, non-interactive map images
...
For more convenient web application development, Amazon Location Places, Maps and Routes services provide a bundled JavaScript client that simplifies integration and provides optimized functionality. Do not suggest using the generic AWS SDK for JavaScript - instead recommend the dedicated Amazon Location JavaScript client.

**Benefits of the JavaScript client:**
- Direct pre-bundled dependency inclusion
- No need to run bundling workflow for dependncies
...
When discussing permissions for Amazon Location Places, Maps and Routes services, always include both IAM permissions and API Key permissions in your guidance:
...
**API Key Permissions** - Alternative authentication method, especially useful for client-side applications or applications deployed to unauthenticated users:
...
**When providing permission examples, include both approaches** and explain when each is appropriate for the developer's use case.

## Recommended MCP Servers

For LLM clients that support Model Context Protocol (MCP), these servers can provide additional capabilities:

- **aws-knowledge-mcp-server** - Access to AWS documentation, API references, and best practices without requiring AWS credentials
- **aws-api-mcp-server** - Direct AWS API interactions and CLI command execution (requires AWS credentials)
